...
VFILES:=Lib/Derivation.v\
  ...
  LL1/ParseTable.v\
  LL1/ParseTableGen.v\
  LL1/Tests/ExampleParseTables.v\
  LL1/Tests/ParserTests.v\
  LL1/Tests/ParseTableTests.v\
  LL1/Proofs/Lemmas.v\
  LL1/Proofs/LL1_Grammar/Determinism.v\
  LL1/Proofs/Parser/Completeness.v\
  LL1/Proofs/Parser/Monotonicity.v\
  LL1/Proofs/Parser/Soundness.v\
  LL1/Proofs/ParseTableGen/mkNullableSet_Correctness.v\
  LL1/Proofs/ParseTableGen/mkFirstMap_Correctness.v\
  LL1/Proofs/ParseTableGen/mkFollowMap_Correctness.v\
  LL1/Proofs/ParseTableGen/mkEntries_Correctness.v\
  LL1/Proofs/ParseTableGen/mkParseTable_Correctness.v\
  LL1/Proofs/ParseTableGen/EndToEnd.v\
  AllStar/Parser.v\
...
install:
	cd "." && for i in $(VOFILES) $(VFILES) $(GLOBFILES) $(NATIVEFILES) $(CMOFILES) $(CMIFILES) $(CMAFILES); do \
	 install -d "`dirname "$(DSTROOT)"$(COQLIBINSTALL)/Vermillion/$$i`"; \
	 install -m 0644 $$i "$(DSTROOT)"$(COQLIBINSTALL)/Vermillion/$$i; \
	done

install-doc:
	install -d "$(DSTROOT)"$(COQDOCINSTALL)/Vermillion/html
	for i in html/*; do \
	 install -m 0644 $$i "$(DSTROOT)"$(COQDOCINSTALL)/Vermillion/$$i;\
	done

uninstall_me.sh: Makefile
	echo '#!/bin/sh' > $@
	printf 'cd "$${DSTROOT}"$(COQLIBINSTALL)/Vermillion && rm -f $(VOFILES) $(VFILES) $(GLOBFILES) $(NATIVEFILES) $(CMOFILES) $(CMIFILES) $(CMAFILES) && find . -type d -and -empty -delete\ncd "$${DSTROOT}"$(COQLIBINSTALL) && find "Vermillion" -maxdepth 0 -and -empty -exec rmdir -p \{\} \;\n' >> "$@"
	printf 'cd "$${DSTROOT}"$(COQDOCINSTALL)/Vermillion \\\n' >> "$@"
	printf '&& rm -f $(shell find "html" -maxdepth 1 -and -type f -print)\n' >> "$@"
	printf 'cd "$${DSTROOT}"$(COQDOCINSTALL) && find Vermillion/html -maxdepth 0 -and -empty -exec rmdir -p \{\} \;\n' >> "$@"
	chmod +x $@
